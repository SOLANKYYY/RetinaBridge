function M=evaluate_dr_lora(runDir)
S=load(fullfile(runDir,'dr_model_lora.mat'));
% Final evaluation only: do not use test performance to select runs.
...
f=fopen(fullfile(runDir,'evaluation.json'),'w'); fprintf(f,'%s',jsonencode(M)); fclose(f); disp(M);
save(fullfile(runDir,'test_predictions.mat'),'truth','pred','scores');
end
