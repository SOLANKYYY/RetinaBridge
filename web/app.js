...
function setBusy(value){busy=value;for(const id of ['run','file','remove','case-id','eye','edit-patient','new-patient'])$(id).disabled=value; $('run').disabled=value||!file||!patientSnapshot;$('run').textContent=value?'Processing image…':'Assess & screen →';$('report').setAttribute('aria-busy',String(value))}
async function status(){try{const r=await fetch('/api/status');if(!r.ok)throw Error();const s=await r.json();$('status').textContent=`${s.runtime} · Model: ${s.model?'loaded':'unavailable'} · Migration test`;}catch{$('status').textContent='Server unavailable. Start python server.py.'}}
...
$('run').onclick=async()=>{if(!file||busy||!patientSnapshot)return;const meta={caseRef:$('case-id').value.trim()||'Not specified',eye:$('eye').value,time:new Date().toLocaleString()};clearReport();setBusy(true);const start=Date.now();message('The model is analysing the image. Please keep this page open.');const timer=setInterval(()=>message(`The model is analysing the image · ${Math.floor((Date.now()-start)/1000)} seconds elapsed. Please keep this page open.`),1000);try{const r=await fetch('/api/screen',{method:'POST',headers:{'Content-Type':'application/octet-stream'},body:file});let d;try{d=await r.json()}catch{throw Error('The server returned an unreadable response. Check the server log.')}if(!r.ok)throw Error(d.error||'Analysis could not finish.');if(!d||typeof d!=='object'||Array.isArray(d))throw Error('Unexpected analysis response.');clearInterval(timer);render(d,meta);message('');}catch(e){clearInterval(timer);message(e.message||'Could not reach the server.',true);}finally{clearInterval(timer);setBusy(false)}};
